...
<title><?php echo $sub_category['sub_cat_name']; ?></title>
</head>
...
<div class="container-fluid splug-container">
    ...
    <ul class="breadcrumb">
                ...
                <li><?php echo $sub_category['sub_cat_name']; ?></li>
            </ul>
    ...
    <div id="splug-content" class="row mobile-category-ajax">
        <!-- left bar section -->
                    ...
                    <div class="filter-sec">
                        <?php $sub = $this->common_model->get_subcategory($sub_category['cat_id']); 
                        if($sub){ ?>
    <div class="box box-with-categories    box-no-advanced">
        <div class="box-heading" id="main-cat-heading"  style="padding-left: 0px !important;">Categories</div>
        <div class="strip-line"></div>  
        <div class="box-content box-category">
                <ul class="accordion" id="accordion-category">
                        <li class="panel">
                                <div id="category63045" class="panel-collapse collapse in" style="clear:both">
                                        <ul>
                                            <?php 
                                            foreach($sub as $s)
                                            {
                                            ?>
                                                <li class="child-category hiddenCategory ">
                                                    <a class="" href="<?php echo base_url('allproduct/'.$s['cat_sub_id']); ?>"><?php echo $s['sub_cat_name']; ?></a>
                                                </li> 
                                    <?php
                                    }
                                    ?>
...
.rating-stars ul > li.star.selected > i.fa {
 ...
 <a class="list-group-item filter-name discounts " style="font-size:18px;font-weight:500;color:black">Discounts</a>
                              <form method="get" action="<?php echo base_url('allproduct/'.$cat_ids); ?>?filter=price=top&order=desc">                                 
                                                            
       
        <div class="list-group-item filter-box-container discounts discounts_filterBox " filter-group="discounts">

      <div id="filter-group4" class="" ><?php 
        ...
        <div class="panel-footer text-right apply-filter">
            <a href="<?php echo base_url('allproduct/'.$cat_ids); ?>" ><button type="button" class="cancel-filter btn">Clear All</button></a>
...
.p1 {
    ...
    <div class="col-sm-6 col-xs-9">
        <div class="pull-left">
      <h1 class="product-title category-name"><?php echo $sub_category['sub_cat_name']; ?></h1> 
      <span class="category-total-item"><?php echo count($products); if(count($products)>1){ ?> items<?php }else{ ?> item<?php } ?></span>
        </div>
    </div>
          <div class="col-sm-6 col-xs-3 short-filter" style="margin-top:-50px;">
          <div class="form-group input-group input-group-sm pull-right short-filter-container ajax-sort">
        <lable style="display: table-cell;padding-right: 10px;position: relative;top: -9px;">SORT BY</lable>
        <select id="input-sort" class="form-control" onchange="location = this.value;" style="margin-top:30px;">

                            <option value="<?php echo base_url('allproduct/'.$cat_ids); ?>?sortby=top&order=desc" <?php if($_REQUEST['sortby']=='top'){ ?> selected=""<?php } ?> onchange="<?php echo base_url('allproduct/'.$cat_ids); ?>?sortby=top&order=desc">Bestseller</option>
                            <option <?php if($_REQUEST['sortby']=='popular'){ ?> selected=""<?php } ?> value="<?php echo base_url('allproduct/'.$cat_ids); ?>?sortby=popular&order=desc" >Popularity</option>
                            <option <?php if($_REQUEST['sortby']=='discount'){ ?> selected=""<?php } ?> value="<?php echo base_url('allproduct/'.$cat_ids); ?>?sortby=discount&order=desc">Discount</option>
                          <!--  <option <?php if($_REQUEST['sortby']=='price' && $_REQUEST['order']=='asc'){ ?> selected=""<?php } ?> value="<?php echo base_url('allproduct/'.$cat_ids); ?>?sortby=price&order=asc">Price: Low to High</option>
                            <option <?php if($_REQUEST['sortby']=='price' && $_REQUEST['order']=='desc'){ ?> selected=""<?php } ?> value="<?php echo base_url('allproduct/'.$cat_ids); ?>?sortby=price&order=desc">Price: High to Low</option> -->
      ...
      <div class="clearfix"></div>
    <div class="sgh-load-more scroll-up-div">
                            <button id="scroll-button-up" type="button">
                                Load Previous Pages
                            </button>
                        </div>
                            <div class="clearfix"></div>
            <input type="hidden" name="list" value="searchpage">
            <div class="row" id="ajax-product-list" style="margin-top:-50px;">
            <?php 
 ...
 <button type="button" class="wishlist-icon wishlist-add-390089" style="display: none;" data-toggle="tooltip" title="Add to Wish List" onclick=""><i class="fa fa-heart"></i></button>
  ...
  <?php $prices = $this->common_model->get_cat_weight($p['pid'],$p['type']); ?>
   <select id="620" checked="checked" name="mySelect" id="mySelect<?php echo $p['pid']; ?>" onchange="return leaveChange($(this).val(),<?php echo $p['pid']; ?>,<?php echo $p['type']; ?>,<?php echo $p['product_id']; ?>)">
        <?php foreach($prices as $pr)
           ...
           <p class="price<?php echo $p['product_id']; ?>" style="margin-top:10px;">
              <?php 
                ...
                <p><?php
                $carts = $this->common_model->get_single_data('cart',array('product_id'=>$p['product_id'],'user_id'=>$this->session->userdata['u_id'],'buy'=>0)); 
                ...
                <?php } }else{ ?>
                        ...
                        <br>
                        <br>
                        <h1 style="text-align:center;font-size: 33px;">Sorry!! No products found for this category!</h1>
                <?php } ?>
    ...
    <div class="filter-menu-icon">
      <button class="sort-btn"><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> sort</button>
  ...
  <div class="sort-filter-popup">
    <div class="sort-filter-container">
                     <label class="check-container">Bestseller           <input id="sort_order_desc" onchange="location = this.value;"  value="<?php echo base_url('allproduct/'.$cat_ids); ?>?sortby=top&order=desc" type="radio" name="sort" <?php if($_REQUEST['sortby']=='top'){ ?> checked="checked" <?php } ?>>
            <span  class="checkmark"></span>
        </label>
                                <label class="check-container">Popularity             <input id="sort_order_asc" onchange="location = this.value;" value="<?php echo base_url('allproduct/'.$cat_ids); ?>?sortby=popular&order=desc" type="radio" name="sort" <?php if($_REQUEST['sortby']=='popular'){ ?>checked="checked" <?php } ?>>
              <span  class="checkmark"></span>
          </label>
                                <label class="check-container">Discount             <input id="discount_desc" onchange="location = this.value;" value="<?php echo base_url('allproduct/'.$cat_ids); ?>?sortby=discount&order=desc" type="radio" name="sort" <?php if($_REQUEST['sortby']=='discount'){ ?>checked="checked" <?php } ?>>
              <span  class="checkmark"></span>
          </label>
                  </div>
  </div>
...
<link href="face/view/theme/ans-theme/stylesheet/searchplug/common.css?v=1641476714" rel="stylesheet" type="text/css" />
<script src="face/view/theme/ans-theme/javascript/searchplug/searchplug.js?v=1641476714" defer="defer"></script>
...
<script>
function leaveChange(val,id,type,pid) {
    $.ajax({
			...
			{
		     $('.price'+pid).html(datas.sub);
		     $('#price'+pid).val(datas.price);
		     $('#weight'+pid).val(val);
			}
	});
	return false;
