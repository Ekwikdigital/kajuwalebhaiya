    ...
    <div id="splug-content" class="row mobile-category-ajax">
        <!-- left bar section -->
                    ...
                    <div class="filter-sec">
                         <?php $sub = $this->common_model->get_subcategory($category['cid']); 
                        if($sub){ ?>
 ...
 <?php $cat_ids=$this->uri->segment(2); ?>
    ...
    <input type="hidden" id="last_selected_filter_group" value="" />
         <a class="list-group-item filter-name discounts " style="font-size:18px;font-weight:500;color:black">Discounts</a>
                 <form method="get" action="<?php echo base_url('products/'.$cat_ids); ?>?filter=price=top&order=desc">                                                  
      ...
      <div id="filter-group4" class="" ><?php 
        ...
        <div class="panel-footer text-right apply-filter">
            <a href="<?php echo base_url('products/'.$cat_ids); ?>" ><button type="button" class="cancel-filter btn">Clear All</button></a>
...
<input type="hidden" id="filter_open_status" value="0" />
    ...
    <div class="col-sm-6 col-xs-9">
        <div class="pull-left">
      <h1 class="product-title category-name"><?php echo $category['cat_name']; ?></h1> 
...
<?php $cat_ids=$this->uri->segment(2); ?>
        <select id="input-sort" class="form-control" onchange="location = this.value;" style="margin-top:30px;">
                            ...
                            <option value="<?php echo base_url('products/'.$cat_ids); ?>?sortby=top&order=desc" <?php if($_REQUEST['sortby']=='top'){ ?> selected=""<?php } ?> onchange="<?php echo base_url('products/'.$cat_ids); ?>?sortby=top&order=desc">Bestseller</option>
                            <option <?php if($_REQUEST['sortby']=='popular'){ ?> selected=""<?php } ?> value="<?php echo base_url('products/'.$cat_ids); ?>?sortby=popular&order=desc" >Popularity</option>
                            <option <?php if($_REQUEST['sortby']=='discount'){ ?> selected=""<?php } ?> value="<?php echo base_url('products/'.$cat_ids); ?>?sortby=discount&order=desc">Discount</option>
 ...
 <button type="button" class="wishlist-icon wishlist-add-390089" style="display: none;" data-toggle="tooltip" title="Add to Wish List" onclick=""><i class="fa fa-heart"></i></button>
           ...
           <p class="price<?php echo $p['product_id']; ?>" style="margin-top:10px;">
              <?php 
                ...
                if($discount==0)
                {
                   ...
                   // $new_price=str_replace(',', '', $price1);
                ?>
                    <input type="hidden" name="weight" id="weight" value="<?php echo $prices[0]['weight']; ?>">
  ...
  <div class="sort-filter-popup">
    <div class="sort-filter-container">
                      <label class="check-container">Bestseller           <input id="sort_order_desc" onchange="location = this.value;"  value="<?php echo base_url('products/'.$cat_ids); ?>?sortby=top&order=desc" type="radio" name="sort" <?php if($_REQUEST['sortby']=='top'){ ?> checked="checked" <?php } ?>>
            <span  class="checkmark"></span>
        </label>
                                <label class="check-container">Popularity             <input id="sort_order_asc" onchange="location = this.value;" value="<?php echo base_url('products/'.$cat_ids); ?>?sortby=popular&order=desc" type="radio" name="sort" <?php if($_REQUEST['sortby']=='popular'){ ?>checked="checked" <?php } ?>>
              <span  class="checkmark"></span>
          </label>
                                <label class="check-container">Discount             <input id="discount_desc" onchange="location = this.value;" value="<?php echo base_url('products/'.$cat_ids); ?>?sortby=discount&order=desc" type="radio" name="sort" <?php if($_REQUEST['sortby']=='discount'){ ?>checked="checked" <?php } ?>>
              <span  class="checkmark"></span>
          </label>
