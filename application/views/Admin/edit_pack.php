...
$product_idd= $get_last_record->pack_product_id;
} else {
   $product_idd= '4890500001';
}
?>
</head>
<body>
  <div class="content-wrapper">
    ...
    <section class="content">
    	<div class="card card-primary">
            <div class="card-body">
            	<form role="form" action="<?php echo base_url('Admin/Packed/edit_submit/'.$product['pack_id']); ?>" method="post" id="vendorFormVal" enctype="multipart/form-data">
            	         <div class="row">
      	<div class="col-md-6">
      ...
      <div class="row">
      		...
      		<div class="form-group">
                ...
                <input type="text" id="product_id" name="product_id" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?php echo $product['pack_product_id']; ?>">
                <span style="color: red;">(Last product id is : <?php echo $product_idd; ?>)</span></span>
                <span style="color: red;"><?php echo form_error('pack_product_id'); ?>  </span>
              </div>
      	</div>
      	<div class="col-md-6">
      		<div class="form-group">
                <label for="inputName">Product Name</label>
                <input type="text" id="p_name" name="p_name" class="form-control" required value="<?php echo $product['pack_name']; ?>">
      ...
      <?php } ?>
      	...
      	<div class="col-md-6">
      		<div class="form-group">
                ...
                <input type="file" name="thumbnail_image" class="form-control" <?php if($product['pack_image']==''){ ?> required <?php } ?> accept=".png, .jpg, .jpeg">
              </div>
              <div class="gg">
              	<?php if($product['pack_image']!=''){ ?>
				<img src="<?php echo base_url(); ?>thumbnail/<?php echo $product['pack_image']; ?>" style="vertical-align:middle; margin-top: 11px;" width="80" height="80" > 
				<span class="btn btn-danger btn-sm" style="cursor:pointer;" onclick="image(<?php echo $product['pack_id']; ?>)">X</span>
				<?php } ?>
			  </div>
              ...
              if($product['product_multi_img']!='')
              {
			     ...
			     ?>
			      <tr class="imagelocationnea<?php echo $i; ?>" style="margin-top: 22px;">
					...
					<span class="btn btn-danger btn-sm" id="ffd<?php echo $i; ?>" style="cursor:pointer;" onclick="deletemulti(<?php echo $i; ?>,'<?php echo $image_name; ?>',<?php echo $product['pack_id']; ?>)">X</span>
...
<div class="row">
        <div class="col-md-6">
      		<div class="form-group">
                ...
                <input type="text" id="p_discount" name="p_discount" placeholder="Product discount" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" value="<?php echo $product['pack_discount']; ?>">
              </div>
        ...
        <div class="col-md-6">
      	...
      	<div class="row">
      		...
      		<div class="form-group">
                ...
                <textarea class="form-control" name="p_description" rows="6" id="p_description"><?php echo $product['pack_description']; ?></textarea>
              </div>
      	</div>
      ...
      <div class="row">
      	<div class="col-md-4">
      		<div class="form-group">
                ...
                <input type="checkbox" name="p_status" <?php if($product['pack_status']==1){ ?>checked <?php } ?> value="1" id="p_status">
              </div>
      	</div>
      	<div class="col-md-4" style="display:none">
      		<div class="form-group">
                <label for="inputProjectLeader">Top Selling</label>
                <input type="checkbox" name="top_sell" value="1" <?php if($product['pack_top_sell']==1){ ?>checked <?php } ?> id="top_sell">
              </div>
      	</div>
      	<div class="col-md-4" style="display:none">
      		<div class="form-group">
                <label for="inputProjectLeader">Popular Searches</label>
                <input type="checkbox" name="popular" value="1" <?php if($product['pack_popular']==1){ ?>checked <?php } ?> id="popular">
              </div>
      	</div>
      </div>
      <div class="form-group">
                            ...
                            <div class="row">
        <div class="col-12">
          <a href="<?php echo base_url('pack-of-two') ?>" class="btn btn-secondary float-right">Cancel</a>
  ...
  function image(pid)
  {
	...
	if (answer)
	{
	        $.ajax({
	                ...
	                url: "<?php echo base_url('Admin/Packed/deleteimage/')?>"+pid,
	                data: {pid: pid},
	...
	function deletemulti(i, image_name, pid)
	{
		...
		if (answer)
		{
		    $.ajax({
               ...
               url: "<?php echo base_url('Admin/Packed/deletecmultiimageea/')?>"+pid,
