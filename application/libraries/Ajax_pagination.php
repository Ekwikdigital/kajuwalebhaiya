...
class Ajax_pagination{
    ...
    function create_links(){
        ...
        // Render the "First" link
				$output .= '<div class="paginaation_my" >
								<ul class="pagination">';
        ...
        function getData(page){  
            $.ajax({
                ...
                success: function(resp){
                    $("'.$this->loading .'").hide();
                    $("'.$this->target .'").html(resp.data);
                }
            });
        }
        </script>';
				$new_var = $anil_link.$output;
        return $new_var;
    }

    function getAJAXlink($count, $text) {
        $pageCount = $count?$count:0;
        return '<li class="page-item"><a class="page-link" href="javascript:void(0);"' . $this->anchor_class . ' onclick="'.$this->link_func.'('.$pageCount.')">'. $text .'</a></li>';
    }
}